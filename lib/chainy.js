...
Chainy.prototype.createChainTask = function(method, opts){
	...
	if ( typeof method === 'string' ) {
		...
		var extensionTaskOptions = (extension.taskOptions || extension.actionOptions || {})
		...
		Object.keys(extensionTaskOptions).forEach(function(key){
			if ( opts[key] == null ) {
				opts[key] = extensionTaskOptions[key]
			}
		})
	}
	...
	if ( opts.args ) {
		...
		if ( injectNormalIndex !== -1 || injectNormalIndex !== -1 ) {
				...
				switch ( extension.type ) {
					case 'action':
						extension.mixinMethod = extension.mixinMethod || function(){
							return this.action(extension.name, {
								args: [Chainy.injectChainDataAsArgument].concat(
