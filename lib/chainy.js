...
Chainy.prototype.createActionTask = function(method, opts){
	...
	task.done(function(err){
		...
		var dataArguments = Array.prototype.slice.call(arguments, 1).filter(function(arg){
			return typeof arg !== 'undefined'
		})

		// Handle appropriatly
		if (err || dataArguments.length === 0 || opts.setArgumentsAsResult === false) {
...
// - actionOptions - An {Object} of action options, forwarded onto {::action}
// 
...
Chainy.addExtension = Chainy.prototype.addExtension = function(){
	...
	if ( me.getValidExtensionName(extension.name) !== extension.name ) {
		var err = me.getInvalidExtensionNameError(extension.name)
		throw err
	}
	...
	if ( extension.type === 'action' ) {
		...
		extension.aliases.forEach(function(alias){
			where[alias] = where[extension.name]
		})
	}
	...
	// If the extension type is utility, don't do anything special to it
		...
		extension.aliases.forEach(function(alias){
			me.mixin(alias, extension.method)
		})
	}
...
Chainy.getInvalidExtensionAliasError = Chainy.prototype.getInvalidExtensionAliasError = function(alias, name){
	var err = new Error(
		'The extension alias '+alias+' is invalid: aliases when stripped of non-alpha characters and lowecased must be equivalant to the extension\'s name: '+name
	)
	return err
}
