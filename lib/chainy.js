(function(){"use strict";
	...
	// Add an action to our runner task queue
	// If it has a result, update the data with the result
	// Give it some optional arguments after the method
	Chainy.prototype.action = function(method, opts){
		if (opts == null)  opts = {}
		this.runner.addTask({
			name: opts.name || 'action '+Math.random(),
			method: function(next){
				Task.create({
					method: method.bind(this),
					args: [this.data].concat(opts.args || []),
	...
	Chainy.mixin = Chainy.prototype.mixin = function(name, extension){
		// Support (extensions) signature
		if ( arguments.length === 1 ) {
			var key, value, extensions = arguments[0]
	...
	Chainy.addExtensions = Chainy.prototype.addExtensions = 
	...
	// Add the most basic of basic plugins
	Chainy.addExtension('set', 'action', function(value, newValue){
		return newValue
	})
