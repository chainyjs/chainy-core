...
Chainy.addExtension = Chainy.prototype.addExtension = function(){
	...
	if ( applies ) {
		...
		switch ( extension.type ) {
			...
			case 'action':
				extension.actionMethod = extension.actionMethod || function(){
				...
				this.mixin(extension.name, extension.actionMethod, extension)
				extension.aliases.forEach(function(alias){
					me.mixin(alias, extension.actionMethod, extension)
				})
				break;

			// If the extension type is utility, don't do anything special to it
			case 'utility':
				this.mixin(extension.name, extension.method, extension)
				extension.aliases.forEach(function(alias){
					me.mixin(alias, extension.method, extension)
				})
				break;
