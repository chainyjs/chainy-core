...
var taskgroup = require('taskgroup'), TaskGroup = taskgroup.TaskGroup

// Internal: Helper to swap an item out of an array
var replaceItemWithArray = function(arr, item, replacement) {
	var index = arr.indexOf(item)
	if ( index === -1 )  return arr
	var before = arr.slice(0, index), after = arr.slice(index+1)
...
Chainy.injectArgumentsAsArgument = 'injectArgumentsAsArgument: '+Math.random()
Chainy.injectArgumentsAsArguments = 'injectArgumentsAsArguments: '+Math.random()
...
//   :args - An {Array} indicating how arguments should be passed to the action, can utilise the special values: `Chainy.injectChainDataAsArgument`, `Chainy.injectExpandedChainDataAsArguments`, `Chainy.injectArgumentsAsArgument`, `Chainy.injectArgumentsAsArguments`
...
Chainy.prototype.createChainTask = function(method, opts, args){
	// Prepare
	var chain = this
	if ( opts == null )  opts = {}
	if ( args == null )  args = []
	...
	if ( opts.setArgumentsAsResult == null ) {
		opts.setArgumentsAsResult = true
	}

	// Ensure args is an array if it exists
	if ( !opts.args ) {
		if ( opts.type === 'action' ) {
			opts.args = [Chainy.injectChainDataAsArgument, Chainy.injectArgumentsAsArguments]
		} else {
			opts.args = [Chainy.injectArgumentsAsArguments]
		}
	}
	...
	task.config.args = replaceItemWithArray(task.config.args, Chainy.injectArgumentsAsArgument, [args])
	task.config.args = replaceItemWithArray(task.config.args, Chainy.injectArgumentsAsArguments, args)
	
	// Handle chain data injection
	task.once('started', function(){
		var data = chain.data
		task.config.args = replaceItemWithArray(task.config.args, Chainy.injectChainDataAsArgument, [data])
		var arrayData = Array.isArray(data) ? data : [data]
		task.config.args = replaceItemWithArray(task.config.args, Chainy.injectExpandedChainDataAsArguments, arrayData)
	})
...
// args - An {Array} of arguments to send to the action
Chainy.prototype.action = function(){
	// Parse arguments
	var method, opts = {}, args = []
	var parseArgument = function(arg){
		var type = arg && (Array.isArray(arg) ? 'array' : (typeof arg))
		switch ( type ) {
			case 'string':
			case 'function':
				method = arg
	...
	Array.prototype.slice.call(arguments).forEach(parseArgument)

	// Prepare options
	if ( opts.type == null )  opts.type = 'action'

	// Create the task with the options
	var task = this.createChainTask(method, opts, args)
...
Chainy.addExtension = Chainy.prototype.addExtension = function(){
	...
	var extension = {}
	var parseArgument = function(arg,index){
		var type = arg && (Array.isArray(arg) ? 'array' : (typeof arg))
		switch ( type ) {
			// name or type
			case 'string':
			...
			case 'function':
				...
				if ( arg.extensionOptions != null )  parseArgument(arg.extensionOptions, index)
				break
				...
				switch ( extension.type ) {
					case 'action':
						extension.mixinMethod = extension.mixinMethod || function(){
							return this.action(extension.name, null, Array.prototype.slice.call(arguments))
						}
						break
